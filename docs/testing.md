...
- **Coverage gate** (`pnpm run test:coverage`): per-file 100% on `packages/*/*/src`. Uncovered lines may be dead code; coverage proves execution, not shipped behavior. `packages/shell/pwsh-local/src` needs real `pwsh`: without it, its suites self-skip and `vitest.config.ts` exempts the file; CI runners enforce coverage. Linux-only procfs and libc execve sources are excluded elsewhere; Linux CI tests and covers them.
