...
- **覆盖率门禁**（`pnpm run test:coverage`）：对 `packages/*/*/src` 按文件要求 100% 覆盖。未覆盖的行可能是死代码；覆盖率只证明执行过，不证明交付行为正确。`packages/shell/pwsh-local/src` 需要真实的 `pwsh`：缺少时相关测试自动跳过，`vitest.config.ts` 豁免该文件；CI runner 执行覆盖率门禁。仅 Linux 使用的 procfs 与 libc execve 源码在其他主机上不计入覆盖率；Linux CI 运行其测试并统计覆盖率。
