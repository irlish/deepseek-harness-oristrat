...
| `assumeReasoning` | `true` | 无任何描述的模型是否按推理模型提供 |
...
`reasoningEfforts` 声明模型可选择的 thinking 等级：每个键都是选择器提供的等级，其值是分派时在协议中发送的拼写，因此 `max: ultra` 可以为拥有自有词汇的网关重命名等级。省略该字段时保留已安装目录条目的能力；对于已安装目录没有描述的模型，则保留该路由的 `assumeReasoning`——它默认开启，因为网关的清单只报告 id 与容量，从不报告模型以哪些等级推理，否则推理强度滑块恰好会在部署手工添加的那些模型上缺席。这类模型提供 pi-ai 自身的默认档位：五个基础等级，`xhigh` 与 `max` 因缺少拼写而不提供。`false` 声明非推理模型，`assumeReasoning: false` 则让该路由列出的所有模型都不提供该滑块。对于 pi-ai 无法识别的端点，`compat` 开关重塑请求——哪个角色携带系统提示词、哪个字段限制输出、thinking 等级如何传递——可逐路由、逐模型配置。条目与已安装目录都没有尺寸的模型，会采用路由的 `defaultContextWindow` 与 `defaultMaxTokens` 回退值。
