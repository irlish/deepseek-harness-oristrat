...
`dsh-browser` 定义 harness（智能体框架）中任何部分可以对用户正在观看的浏览器面板做什么：导航、把页面读成携带引用的文本、对元素执行操作、截取图像、读取控制台输出、求值脚本。这些定义同时固定了每个提供方、工具与客户端共享的词汇——`@e12` 元素引用、`BROWSER_*` 失败码，以及请求与结果记录。把它与提供方一起组合，浏览器工具才能驱动该面板；目前桌面应用挂载了唯一的提供方。只挂载本包不会驱动任何东西：没有提供方时 `ctx.browser` 不存在，也不会有任何 `browser_*` 工具。
...
只有把提供方与消费方一起挂载，配置文件才能驱动浏览器。桌面应用拥有内嵌面板及通往它的命令通道，因此由它来挂载提供方：
...
- [browser-desktop](../browser-desktop/README.zh.md) —— 驱动桌面客户端所显示内嵌面板的提供方。
...
- **仅为抽象**——挂载本包只是注册服务，属性背后没有提供方，因此组合必须存在提供方配置行才能驱动浏览器；`dsh-browser-desktop` 是唯一的提供方，且只有桌面应用组合它。
