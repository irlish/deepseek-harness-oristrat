...
const { mockConnect, mockClose, mockListTools, mockCallTool, mockSetNotificationHandler,
  mockClientOptions, MockClient } = vi.hoisted(() => {
  ...
  class MockClient {
    ...
    constructor(_info: unknown, options: { listChanged: { tools: { onChanged: () => void } }; versionNegotiation: { mode: string } }) {
      mockSetNotificationHandler('notifications/tools/list_changed', options.listChanged.tools.onChanged)
      mockClientOptions(options.versionNegotiation)
    }
    getServerCapabilities = () => ({ tools: {} })
    getInstructions(): string | undefined { return undefined }
  }
  return { mockConnect, mockClose, mockListTools, mockCallTool, mockSetNotificationHandler, mockClientOptions, MockClient }
})
...
describe('apply (plugin lifecycle)', () => {
  ...
  it('uses streamable-http config path', async () => {
    ...
    expect(mockConnect).toHaveBeenCalled()
    expect(mockClientOptions).toHaveBeenLastCalledWith({ mode: 'auto' })
    expect(ctx.tools.get('mcp__web__remote')).toBeDefined()
  })

  it('uses the SDK legacy handshake for a configured Streamable HTTP server', async () => {
    await apply(ctx, {
      transport: 'streamable-http',
    ...
    expect(mockClientOptions).toHaveBeenLastCalledWith({ mode: 'legacy' })
  })
  ...
  it('resolves authorizationEnv into a Bearer header on the transport', async () => {
    ...
    const transport = vi.mocked(StreamableHTTPClientTransport)
