...
| `@deepseek-ai/dsh-api-gui-repo` | no | Read-only repository environment Remote BFF: branch, working-tree change totals, and remote sources over the subprocess layer |
| `@deepseek-ai/dsh-api-gui-terminal` | no | Browser-interactive terminal Remote BFF over the subprocess PTY layer |
...
| Package | Config | Description |
|---|---|---|
| `@deepseek-ai/dsh-browser-desktop` | yes | Desktop provider for the browser automation seam (ctx.browser): drives the embedded browser pane the desktop client shows through a brokered command channel, with protocol-level input that never takes the operating system's keyboard or pointer |
| `@deepseek-ai/dsh-tool-browser` | no | Browser automation tools over the ctx.browser seam: navigate, observe, act, screenshot, console, state, and eval against the embedded browser pane, without taking the operating system's keyboard or pointer |
...
| `@deepseek-ai/dsh-client-ui-browser-panel` | no | Browser right-sidebar embedded browser panel over the desktop WebContentsView bridge |
...
| `@deepseek-ai/dsh-client-ui-repo-panel` | no | Browser Session-header repository environment menu over the gui-repo Remote namespace |
...
| `@deepseek-ai/dsh-client-ui-terminal-panel` | no | Browser bottom-panel interactive terminal (xterm.js) over the gui-terminal Remote namespace |
...
| `@deepseek-ai/dsh-context-oristrat-msce-norms` | yes | Always-on Oristrat MSCE engine development norms as a system-prompt section |
...
| Package | Config | Description |
|---|---|---|
| `@deepseek-ai/dsh-guard-msce-gate` | no | Hard MSCE gates: discovery before code mutation, submission gate before git handoff |
