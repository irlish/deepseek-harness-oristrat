...
Use the sidebar to browse Workspaces and their Sessions, reorder them, and start new ones; use the picker in the Session Intent hero to choose a Workspace for a new session. The grouped tree leads with the **Recent Sessions** bucket — the Ungrouped account holding Sessions outside every Workspace. It is expanded by default, its ＋ creates a Session with no Workspace (the Host lands it on its default working directory), and New Session also falls into this bucket when no Workspace resolves. The archives-only view lists archives rather than that start surface, so there it appears only while archived Sessions sit outside every Workspace. The hero picker lists a **No workspace** entry (zh 不关联工作区) above the Workspace rows: picking it starts a Session outside every Workspace, and with zero Workspaces the anchor gesture opens that two-entry menu instead of jumping straight into the directory flow. A blank Session without an owning Workspace stays conversable — its chip reads **No workspace** (zh 未关联工作区) and the composer stays live. An open Workspace shows five idle, non-blank Sessions by default. Running Sessions, including parents with running children, remain visible in their ordered positions without using that quota; the selected blank **New Session** is also an extra row until its first prompt. Each **Show more** click reveals up to five more idle Sessions; after the final batch, **Show less** restores the initial rows while keeping running Sessions visible. Closing and reopening the Workspace also restores this folded projection.
