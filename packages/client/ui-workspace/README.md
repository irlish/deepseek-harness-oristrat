...
Use the sidebar to browse Workspaces and their Sessions, reorder them, and start new ones; use the picker in the Session Intent hero to choose a Workspace for a new session. The grouped tree always leads with the **Recent Sessions** bucket — the Ungrouped account holding Sessions outside every Workspace. It is expanded by default, its ＋ creates a Session with no Workspace (the Host lands it on its default working directory), and New Session also falls into this bucket when no Workspace exists at all. The hero picker lists a **No workspace** entry (zh 不关联工作区) above the Workspace rows: picking it starts a Session outside every Workspace, and with zero Workspaces the anchor gesture opens that two-entry menu instead of jumping straight into the directory flow. A blank Session without an owning Workspace stays conversable — its chip reads **No workspace** (zh 未关联工作区) and the composer is live. An open Workspace shows five non-blank Sessions by default and keeps the selected blank **New Session** as one provisional extra row until its first prompt. **Show more** reveals the hidden remainder; closing and reopening the Workspace restores this folded projection.
