...
import {
  IconChevronDownOutlineRegular, IconCodeOutlineRegular, IconNewChatOutlineMedium, IconNewChatOutlineRegular,
  IconPanelLeftOutlineRegular, IconSparkleRegular, isDarwinDesktop, Menu, OristratBrand, OristratMark, ShortcutKeys, Tooltip,
...
function ModeSwitcher({ setMode, useMode, t }: ModeSwitcherProps) {
  ...
  const entry = (id: SidebarMode): MenuEntry => ({
    id,
    icon: id === 'work' ? <IconSparkleRegular size={16} /> : <IconCodeOutlineRegular size={16} />,
    label: (
  ...
  const anchor = (
    ...
    >
      {mode === 'work' ? <IconSparkleRegular size={14} /> : <IconCodeOutlineRegular size={14} />}
      <span className={css.modeChipName}>{t(mode === 'work' ? 'mode.work' : 'mode.coding')}</span>
      <IconChevronDownOutlineRegular size={14} className={css.modeChipChevron} />
    </button>
  )
  return (
    <Menu
...
export function SidebarRoot({
  ...
  // Rail resting state is the Oristrat mark; hovering swaps in the panel icon
      ...
      >
        {!wide && !windowsTitlebar && (
          <span className={css.railMark} aria-hidden="true">
            {renderSlot('sidebar.brand.mark', { size: 24 }, { fallback: <OristratMark size={24} /> })}
          </span>
        )}
