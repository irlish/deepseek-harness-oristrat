...
function mountShell({
  collapsed = false, width = 300, mode = 'coding', shortcuts = [],
}: {
  collapsed?: boolean
  width?: number
  mode?: 'coding' | 'work'
...
describe('SidebarRoot shell', () => {
  it('advertises the effective new-session binding', () => {
    ...
    cleanup()
    render(<HeaderLeadingControls toggleSidebar={vi.fn()} startSession={vi.fn()} selectPanel={vi.fn()} setMode={vi.fn()} t={t}
      ...
      usePanels={select => select([])} useMode={select => select('coding')} useShortcuts={select => select([shortcut])} />)
  ...
  it('renders the Oristrat brand fallback when no package fills the slots', () => {
    ...
    expect(container.querySelector('[aria-label="Oristrat AI STEM"]')).not.toBeNull()
    expect(screen.getByText('Oristrat AI')).toBeTruthy()
  ...
  ])('keeps the Oristrat wordmark with build metadata %j', (environment, expected) => {
