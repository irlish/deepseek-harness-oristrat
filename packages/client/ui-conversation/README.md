...
The Oristrat deployment reads the committed `oristrat-msce-norms` mode through a live settings snapshot. Composer accessory extension entries receive the current Session input zone only in Work mode; the statistics dock renders with that zone in either mode, and owner-supplied accessory content remains available in either mode.
