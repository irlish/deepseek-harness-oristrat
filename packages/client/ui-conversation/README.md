...
The Oristrat deployment reads the committed `oristrat-msce-norms` mode through a live settings snapshot. Composer accessory and dock extension entries receive the current Session input zone only in Work mode; owner-supplied accessory content remains available in either mode.
