...
describe('ModelsSection', () => {
  ...
  it('fetches direct-provider models and reports empty, refused, and failed discovery', async () => {
    const discoverModels = vi.fn<ModelsOperations['discoverModels']>()
      .mockResolvedValueOnce({ kind: 'found', models: [
        { id: 'known' }, { id: 'new', name: 'New model' }, { id: 'unnamed' },
      ] })
      .mockResolvedValueOnce({ kind: 'refused', message: 'Catalog refused' })
      .mockResolvedValueOnce({ kind: 'found', models: [] })
      .mockRejectedValueOnce(new Error('Network unavailable'))
      .mockRejectedValueOnce('Unexpected failure')
      .mockResolvedValueOnce({ kind: 'found', models: [{ id: 'known' }] })
    const onChange = vi.fn()
    render(<DeepSeekModelsEditor
      models={[{ id: 'known' }, { id: 12 }]} overridden={false}
      defaultContextWindow={undefined} defaultMaxTokens={undefined}
      t={t} disabled={false} onChange={onChange}
      operations={{
        describeCredential: vi.fn(), storeCredential: vi.fn(), removeCredential: vi.fn(),
        writeSettings: vi.fn(), discoverModels,
      }}
      probe={{ settingsNs: 'llm-deepseek', provider: 'deepseek', baseURL: 'https://example.test', api: 'openai-completions', apiKey: 'test-key' }}
    />)
    const fetch = screen.getByRole('button', { name: en.fetchUpstream })
    fireEvent.click(fetch)
    await waitFor(() => { expect(onChange).toHaveBeenCalledWith([
      { id: 'known' }, { id: 12 }, { id: 'new', name: 'New model' }, { id: 'unnamed' },
    ]) })
    expect(discoverModels).toHaveBeenCalledWith('llm-deepseek', {
      provider: 'deepseek', baseURL: 'https://example.test', api: 'openai-completions', apiKey: 'test-key',
    })
    expect(screen.getByText(en.fetchResult)).toBeTruthy()

    fireEvent.click(fetch)
    await screen.findByText('Catalog refused')
    fireEvent.click(fetch)
    ...
    fireEvent.click(fetch)
    await screen.findByText(en.fetchResult)
    expect(onChange).toHaveBeenCalledTimes(1)
  })

  it('omits absent probe fields from a direct-provider catalog request', async () => {
    const discoverModels = vi.fn<ModelsOperations['discoverModels']>()
      .mockResolvedValue({ kind: 'found', models: [] })
    render(<DeepSeekModelsEditor
      models={[]} overridden={false} defaultContextWindow={undefined} defaultMaxTokens={undefined}
      t={t} disabled={false} onChange={vi.fn()}
      operations={{
        describeCredential: vi.fn(), storeCredential: vi.fn(), removeCredential: vi.fn(),
        writeSettings: vi.fn(), discoverModels,
      }}
      probe={{ settingsNs: 'llm-deepseek', baseURL: '' }}
    />)
    fireEvent.click(screen.getByRole('button', { name: en.fetchUpstream }))
    await screen.findByText(en.fetchEmpty)
    expect(discoverModels).toHaveBeenCalledWith('llm-deepseek', {})
  })

  it('removes an optional display name and explains a blocked catalog fetch', () => {
    const onChange = vi.fn()
    render(<DeepSeekModelsEditor
      models={[{ id: 'known', name: 'Old name' }]} overridden={true}
      defaultContextWindow={undefined} defaultMaxTokens={undefined}
      t={t} disabled={false} onChange={onChange}
      operations={{} as ModelsOperations}
      probe={{ settingsNs: 'llm-deepseek' }} probeBlocked="fetchEmpty"
    />)
    const fetch = screen.getByRole('button', { name: en.fetchUpstream })
    expect(fetch.hasAttribute('disabled')).toBe(true)
    expect(fetch.getAttribute('title')).toBe(en.fetchEmpty)
    fireEvent.change(screen.getByLabelText(`${en.modelName} 1`), { target: { value: '' } })
    expect(onChange).toHaveBeenCalledWith([{ id: 'known' }])
  })
