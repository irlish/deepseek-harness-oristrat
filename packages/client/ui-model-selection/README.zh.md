...
与 `ui-conversation` 及命令包一起挂载本插件；composer 随即在待处理指示器旁显示模型位，`/model` 则以弹窗打开同一份目录。模型位菜单打开期间，`↑`／`↓` 在所显示面板的行间移动焦点，`Tab` 选定聚焦行，Escape 与 `Shift+Tab` 先退出已下钻的模型列表，否则关闭并回到触发器。下钻落在正在使用的那一行，返回则落在打开该列表的格子上。所选模型可用时，composer 显示目录中的模型名称；模型或提供方被删除时（包括账号退登），改为显示已保存的 `provider/model` ID。保存的提供方、模型和推理强度均保持不变。
...
模型按提供方分组，DeepSeek 账号排第一，DeepSeek 排第二，第三方提供方保持目录原有顺序。导航箭头使用 `--dsw-alias-menu-icon` 文本色。composer 菜单是一个紧凑选择器：模型行之后是推理强度区块，其标题与当前等级共占一行，下方为 22px 高的滑块。区块宽 224px，上下内边距分别为 7px 与 9px，整张卡片保持在 260x120px 以内。适配器每公布一个等级就有一个档位，蓝色填充到圆点为止；最高档位会填满整条胶囊并让渐变延伸到末端，跟随提供方默认值时轨道为空、圆点呈空心。14px 圆点位于 22px 胶囊之内：最外侧档位距两端各 15px，即胶囊半径加圆点半径，因此圆点停在两端时与圆弧内切。最高可用等级显示蓝紫渐变轨道与轻量闪光动画，且不再单独占一行说明；减少动态效果偏好下保持静态。拖动圆点会按住经过的等级——圆点、已填充轨道与等级名称随之变化——并在松手时经由与其他选择相同的目录路径提交一次；方向键、Home、End 则在每次按键落在的档位上提交。推理强度提交成功后菜单保持在已提交的档位上，因此拖动或按键落在一个等级上不会把滑块收走；成功选定模型后菜单关闭。卡片只在明确的关闭动作下关闭——点击卡片外部、点击触发器或按 Escape：焦点离开卡片不会关闭它，拖出轨道以外的拖动会一直持有指针（即使拖到窗口外），因此这两种情况都不会让卡片消失。`/model` 弹窗显示提供方名称与目录说明；其中两个内置 DeepSeek 模型的说明使用当前语言，外部提供方说明保持原文。弹窗应用所选模型的默认推理强度；composer 滑块随后可以选择任一已公布的推理强度。适配器没有推理元数据时不显示滑块与推理强度值；不存在任意推理强度输入。
