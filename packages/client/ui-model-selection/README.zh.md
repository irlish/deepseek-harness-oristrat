...
模型按提供方分组。composer 菜单显示模型名称，并在模型列表下方提供推理强度滑块：适配器每公布一个等级就有一个档位，已选档位之前的轨道填充显示；会话跟随提供方默认值时滑块呈空心圆点并标注提供方默认说明。拖动圆点或使用方向键、Home、End 都会经由与其他选择相同的目录路径立即提交；成功提交推理强度后菜单关闭。`/model` 弹窗显示提供方名称与目录说明；其中两个内置 DeepSeek 模型的说明使用当前语言，外部提供方说明保持原文。弹窗应用所选模型的默认推理强度；composer 滑块随后可以选择任一已公布的推理强度。适配器没有推理元数据时不显示滑块与推理强度值；不存在任意推理强度输入。
