...
Mount this plugin alongside `ui-conversation` and the commands package; the composer then shows the model seat next to the pending indicator, and `/model` opens the same directory as a popup. While the seat's menu is open, `↑`/`↓` move focus across the rows of the shown pane, Tab settles the focused row, and Escape and `Shift+Tab` leave the drilled model list first and otherwise close back to the trigger. Drilling lands on the row of the value in use, and going back lands on the cell that opened the list. The composer shows the catalog name while the selected model is available, and its saved `provider/model` ID when the model or provider is removed, including account sign-out. The stored provider, model, and reasoning effort remain unchanged.
...
Models stay grouped by provider, with DeepSeek Account first and DeepSeek second; third-party providers retain their catalog order. Navigation chevrons use `--dsw-alias-menu-icon`. The composer menu is one compact picker: the model row, then a reasoning-effort block whose caption and current level share one line above a 22px slider. The block is 224px wide with 7px/9px vertical padding, so the whole card stays under 260x120px. The slider offers one stop per adapter-advertised level and fills blue up to the knob; the highest stop fills the pill with its gradient running to the end, and the provider default leaves the track empty under a hollow knob. Its 14px knob sits inside the 22px pill: the outermost stops sit 15px from either end, which is the pill's radius plus the knob's, so a parked knob stays tangent inside the rounded end. The highest offered level uses a blue-to-violet track with subtle animated sparkles and no caption line of its own; reduced-motion preference keeps it static. A pointer drag holds the level it crosses — the knob, the filled track, and the level name follow it — and commits once on release through the same directory path as any other selection; the arrow, Home, and End keys commit the stop each key lands on. A settled effort commit leaves the menu open on the committed stop, so a drag or a key that lands on a level does not take the slider away; a settled model pick closes the menu. The card closes only on an explicit dismissal — a click outside it, a click on the trigger, or Escape: focus moving off the card does not close it, and a drag that reaches past the track keeps its pointer even outside the window, so the card survives both. The `/model` popup shows provider names and catalog descriptions; it localizes the two built-in DeepSeek descriptions and leaves external provider descriptions verbatim. The popup applies the selected model's default effort; the composer slider can then choose any advertised effort. An adapter without reasoning metadata leaves the slider and effort value absent; there is no arbitrary effort input.
