...
Desktop uses the DashScope WebSearch MCP tool for model-initiated search. Set `DASHSCOPE_API_KEY` in `$DSH_HOME/.env`; the Desktop Host resolves it when connecting to the MCP server. The MCP entry injects `credentials`, so it activates only after that provider exists: reading the key before then fails the entry's first connection, and a client that never connected never reports close, which stops the reconnect supervisor for the rest of the run. The inherited preset `web_search` tool and the DeepSeek search provider are disabled in Desktop, while `web_fetch` remains available. If the key is missing or the MCP server is offline, Desktop still starts and exposes no default search tool until the MCP connection succeeds. The [WebSearch MCP decision](../../.agents/notes/implemented/feature/2026-09-22-oristrat-thinking-slider-websearch-mcp-repo-browser-panels.md) records the deployment choice.
...
The right Sidebar embeds one `WebContentsView` per window for human browsing, and agent automation drives that same view. The dsh Host child sends browser debugging-protocol commands over the shell control channel (`DESKTOP_HOST_PROTOCOL_VERSION` 4), and the main process brokers each one against a fixed method allowlist, attaching the debugger on the first command and disabling background throttling while attached. No command synthesizes operating-system input or focuses the view, so a user typing in the application keeps typing there while a run proceeds. The pane is revealed on demand through `dsh-desktop:browser-reveal` and reports what it is doing through `dsh-desktop:browser-activity`. The [sidebar browser automation Agent Note](../../.agents/notes/implemented/feature/2026-09-24-oristrat-sidebar-browser-agent-automation.md) records the seam, its bounds, and the desktop-only scope.

Electron chooses typed English or Chinese shell copy from its application locale and falls back to English. Menus, native dialogs, the startup page, and the plugin-management renderer use the same locale payload; the repository Client UI i18n gate checks these desktop sources. The startup page loads `renderer/locales.js` for its plain-browser preview without a preload; the packaged application still obtains its copy from the shell locale payload.
