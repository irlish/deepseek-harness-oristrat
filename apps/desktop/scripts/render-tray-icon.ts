/**
 * Render the Windows tray icon from the Oristrat application artwork.
 *
 * The tray shows the icon at 16 logical pixels, so Windows picks one of the
 * bundled bitmaps by display scale. Each size is resized from the source PNG.
 * The committed `resources/tray-windows.ico` is the output; rerun
 * `pnpm run render:tray-icon` in `apps/desktop` after changing the app icon.
 */
...
/** Application artwork source and committed output of the tray icon. */
export const TRAY_ICON_PATHS = {
  source: fileURLToPath(new URL('../build/icon.png', import.meta.url)),
...
/**
 * Resize the Oristrat application artwork at each tray size.
 * @param artwork - Square application PNG.
 * @param sizes - Bitmap edges to render.
 * @returns PNG entries in the given order.
 */
export async function renderTrayIconEntries(artwork: Buffer, sizes: readonly number[] = TRAY_ICON_SIZES): Promise<IcoEntry[]> {
  return Promise.all(sizes.map(async size => ({
    size,
    png: await sharp(artwork).resize(size, size).png().toBuffer(),
  })))
}
