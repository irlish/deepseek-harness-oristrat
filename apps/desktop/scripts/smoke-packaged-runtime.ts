...
if (values.unsigned && !windows && !target.startsWith('mac-')) {
...
const application = windows ? join(artifacts, 'win-unpacked')
  : join(artifacts, target === 'mac-arm64' ? 'mac-arm64' : 'mac', 'Oristrat AI Stem.app', 'Contents')
const resources = join(application, windows ? 'resources' : 'Resources')
const executable = windows ? join(application, 'Oristrat AI Stem.exe') : join(application, 'MacOS', 'Oristrat AI Stem')
