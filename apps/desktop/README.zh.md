...
Oristrat 的 OpenAI 兼容网关接受推理强度，但拒绝 pi-ai 原本为推理模型选择的 `developer` 消息角色。Web 应用 bundle 通过当前 profile 配置提供 `oristrat-official` 路由，其中包含 `compat.supportsDeveloperRole: false`，使新安装的系统提示词使用 `system` 角色。Desktop 在 Host 启动前为旧版设置里缺少推理等级的模型补齐等级，并为既有 profile 行补齐缺失的兼容字段。官方 Settings 服务将旧版主目录中的 `settings.yaml` 导入 profile，同时保留其他用户选项。共享加载页在 Host 和客户端插件初始化时继续显示 Oristrat 动态标志及 STEM 字标。
...
Windows 在整个运行期间常驻托盘图标。悬停提示为产品名，单击显示并聚焦窗口，右键菜单提供壳语言下的"打开 Oristrat AI Stem"和"退出 Oristrat AI Stem"。首次隐藏前复用更新弹窗，显示“正在运行的任务不会中断，可在系统托盘中重新打开窗口”和“确认”按钮。确认后隐藏窗口，并在 Electron userData 下写入 `background-close-confirmed`；Esc、关闭弹窗或加载失败均保持主窗口可见，不记录确认。重复关闭请求会聚焦已有壳弹窗。覆盖更新保留标记，卸载删除标记。旧的 `background-notice-shown` 标记不会跳过此确认。关闭窗口不发送系统通知。托盘位图是 `resources/tray-windows.ico`，由 `pnpm run render:tray-icon` 从 `build/icon.png` 按 16、20、24、32、40、48、64 像素分别渲染，打包为 `resources/tray.ico`。macOS 不提供菜单栏图标。

所有普通退出入口——⌘Q、应用菜单、Dock 菜单、Windows 托盘和标题栏"应用程序"菜单，以及关闭强制更新窗口或欢迎窗口引起的退出——都先向 Host 查询退出会中断什么。Host 通过私有 IPC 通道回答两项事实：与更新重启检查同一口径的运行中任务（运行中的 agent，包括子代理和等待审批的回合、排队消息、运行中或停止中的后台任务），以及本次运行中已加载会话里由 `workspace/session-activity` 的 `schedule` family 报告的已挂定时器的提醒。两项都没有时直接退出，不弹框。否则弹出一个没有父窗口的原生消息框——隐藏的窗口保持隐藏——标题为**退出 Oristrat AI Stem？**，正文为三种本地化说明之一：正在运行的任务将会中断、应用关闭期间定时任务不会运行，或两者兼有。"退出"是默认按钮，Esc 等同"取消"；macOS 上"取消"在"退出"左侧，Windows 上"退出"在"取消"左侧，Windows 任务对话框显示应用图标且不跟随应用主题、始终为浅色。Host 尚未就绪或已失败时不可能有任务在跑，直接退出。查询失败或 Host 超过两秒截止时间未答复，按运行中任务处理。弹框打开期间，再次请求退出只会并入同一弹框而不叠加新弹框（macOS 上还会把它提到前面；Electron 不暴露 Windows 任务对话框的句柄）；任务开始或结束不会改变文案；点"退出"不再重新查询即停止应用；点"取消"不发生任何变化。取消由关闭欢迎窗口引起的退出时，欢迎窗口会重新显示。
...
托盘图标从 Oristrat 应用图案分别缩放到各个显示比例；应用和安装器图标保持原有比例。

## 关键技术决策

Oristrat 六边形网络标识位于 `build/icon.png` 和 `build/icon.icns`。electron-builder 将 ICNS 用于 macOS，将 PNG 用于 Windows 应用、安装程序、卸载程序和“关于”面板。独立欢迎页通过 `renderer/assets/welcome-brand.svg` 嵌入同一标识；Web 客户端及桌面端内嵌页面在明暗两种主题下都使用 Oristrat 图标。Windows 安装页面使用 `installer/assets/` 中匹配明暗主题的 Oristrat 图案；在 macOS 上运行 `swift apps/desktop/scripts/render-oristrat-installer-brand.swift` 可从应用图标重新生成这些 PNG。卸载程序的欢迎和完成页共用 `installer/assets/uninstaller-sidebar.png`，准备阶段将其转换为 164×314 BMP。
...
macOS ICNS 包含最高 1024 像素的圆角应用图标。它是扁平图标，并非 Icon Composer 文档。Apple 的[应用图标指南](https://developer.apple.com/design/human-interface-guidelines/app-icons)要求向 Icon Composer 提供未遮罩的图层；这些输入需要在 macOS 上单独导出，不能复用已做圆角的 ICNS 图案。发布前须在支持的 macOS 版本中验收 Finder 和 Dock 的显示效果。
...
Desktop 携带独立的 Python、Node.js 和 pnpm 分发包。Python 包含 numpy、pandas、python-docx、python-pptx、openpyxl、Pillow、lxml、XlsxWriter 及其完整依赖。`load_workspace_dependencies` 工具首次使用时，将该产物离线安装到 `$DSH_HOME/dsh-runtimes/dsh-primary-runtime`（通常为 `~/.oristrat/dsh-runtimes/dsh-primary-runtime`），并返回解释器、pnpm 脚本和库目录的绝对路径，以及记录内置分发包名称与版本的 `pythonDistributions`。版本报告不包含用户自行安装的包。Office 任务默认使用这些库，用户或工作区指令指定其他环境时遵循其要求。pnpm 脚本通过返回的 Node 可执行文件运行。返回的 Node 库目录为随包交付的库预留，不是 pnpm 的全局安装目录。
...
首个致命弹窗打开前，Electron 会向平台日志目录（`app.getPath('logs')`：macOS 为 `~/Library/Logs/Oristrat AI Stem`，Windows 与 Linux 为应用 `userData` 目录下的 `logs`）写入一份崩溃报告，最多等待写入一秒；写入缓慢或失败时弹窗不带路径。文件 `crash-<UTC 时间>-<source>.log` 记录来源（`host` 为 Host 退出、`web-boot` 为渲染进程启动失败、`renderer` 为渲染进程或文档失败、`main` 为壳自身错误）、后端是否已就绪、应用与运行时版本、包含可枚举属性与 cause 链的错误（截至 256 KiB）、Host 在退出前通过 IPC 报告启动失败时自己的 inspect 错误（最多 64 KiB），以及主窗口最近的 error 级 console 输出（最多 64 KiB）。因此 Host 退出报告包含保留的 stderr 尾部，其中可能含有插件输出。关闭过程中的致命失败只写报告、不弹窗。平台支持时文件仅所有者可读；启动时保留最新十份报告并删除更早的，不触碰目录中的其他文件。
...
Desktop 在 Host 启动后、打开工作区前检查模型 API Key 引用是否已配置。没有已配置的密钥时，欢迎窗口提供 [API Key 页面](https://www.figma.com/design/jRBBK7zBgcszdVWQ0Fh5J8/Harness?node-id=2138-44626)，说明文案明确指向 Oristrat 模型。“保存并继续”通过现有凭证服务写入默认模型提供方配置的引用，包括 bundle 中的 `oristrat-official` 路由，然后打开工作区。未配置默认模型提供方的 profile 在启用 DeepSeek 原生提供方时沿用其引用。“稍后配置”打开工作区，但不保存草稿或完成标记；下次进程启动时会重新检查凭证。“返回登录”回到入口并清空未保存的密钥和校验提示。保存或打开工作区期间，按钮保持原文案并禁用竞争操作。Desktop preload 标记使 Web 凭证弹窗不再显示，同时保留模型设置页和欢迎须知。
...
本地 macOS 构建只需在忽略的 `.env.macos` 中写入 `DSH_DESKTOP_APP_ID=com.oristrat.ai.stem`，然后运行 `pnpm --dir apps/desktop run package:mac:arm64:unsigned`。命令在 `.desktop-build/targets/mac-arm64/unsigned-artifacts/` 生成未签名 DMG 和 ZIP，并对打包运行时执行冒烟检查。该模式不嵌入发布更新策略，也不签名或公证，仅供本地测试，不能作为发布产物上传。

发布打包命令在构建与下载前检查应用 ID、更新地址和该模式需要的签名配置，随后探测本次运行要用的外部工具：归档读取工具，以及 Windows 目标的安装器编译器。已签名 macOS 构建检查身份、Team ID、一套完整公证凭据、`CSC_LINK` 指定的可读本地 p12 文件、显式配置的 `CSC_KEY_PASSWORD`，以及引用的 API Key 和钥匙串文件；Windows 发布构建检查公开代码签名证书、SignTool 文件、容器名称和 PIN 格式。仅准备 Windows 资源或两个平台的显式未签名打包不要求签名凭据。配置检查不验证 PIN 是否正确、Token 是否登录、钥匙串是否解锁或 Apple 是否接受凭据；实际签名与公证负责这些检查。`--build-version auto` 会访问目标 bucket，`--check` 下同样如此。单独运行相同检查：
