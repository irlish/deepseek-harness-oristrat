...
it('retains one shared resource consumer in the Desktop Web profile', () => {
  ...
  try {
    ...
    const rows = composeEntries([
      ...
      profile.patches,
      loadOverlayPatches('dsh desktop', fileURLToPath(new URL('../../desktop-host/config/desktop.cordis.patch.yml', import.meta.url))),
    ...
    expect(rows.filter(row => row.name === '@deepseek-ai/dsh-mcp-client')).toMatchObject([
      { id: 'mcp-client-dashscope-websearch', inject: ['tools', 'credentials'] },
    ])
