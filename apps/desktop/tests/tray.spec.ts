...
it('shows the application icon with its name as the tooltip and an Open / Quit menu', () => {
  ...
  expect(f.native.setToolTip).toHaveBeenCalledWith('Oristrat AI Stem')
  expect(labels(native.menus[0]!)).toEqual(['Open Oristrat AI Stem', 'separator', 'Quit Oristrat AI Stem'])
...
it('relabels the menu in the current locale and ignores relabel after disposal', () => {
  ...
  f.setLocale('zh')
  f.tray.relabel()
  expect(labels(native.menus[1]!)).toEqual(['打开 Oristrat AI Stem', 'separator', '退出 Oristrat AI Stem'])
  f.tray.dispose()
  f.tray.dispose()
