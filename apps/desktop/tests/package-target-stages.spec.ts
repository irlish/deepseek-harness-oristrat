...
it('builds and smokes an unsigned macOS artifact without a signing or notarization stage', async () => {
  const { run, stages } = supervisor()
  await packageTarget(parseDesktopPackageInvocation(['mac-arm64', '--unsigned'], 'darwin', 'arm64'), environment, run)
  expect(stages.at(-2)).toContain('exec electron-builder --config electron-builder.config.mjs --mac --arm64')
