...
describe('installer preparation preserves application dependencies', () => {
  ...
  it.each(['win32', 'darwin'] as const)('keeps electron-builder responsible for node_modules on %s', async (platform) => {
    ...
    try {
      ...
      expect(readFileSync(config.win.icon)).toEqual(readFileSync(new URL('../build/icon.png', import.meta.url)))
      expect(readFileSync(config.mac.icon)).toEqual(readFileSync(new URL('../build/icon.icns', import.meta.url)))
  ...
  it('names unsigned Windows artifacts so they cannot pass for release builds', async () => {
    ...
    expect(config.artifactName).toBe('oristrat-ai-stem-${version}-${os}-${arch}-unsigned.${ext}')
  })
