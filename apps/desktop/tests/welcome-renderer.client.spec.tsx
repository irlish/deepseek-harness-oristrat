...
describe('desktop welcome presentation', () => {
  it.each(['zh-CN', 'en'])('renders the %s entry and API-key step', async (language) => {
    ...
    const brand = readFileSync(join(import.meta.dirname, '../renderer/assets/welcome-brand.svg'), 'utf8')
    expect(brand).toContain('Oristrat AI')
    expect(brand).toContain('STEM')
    expect(brand).toContain('data:image/png;base64,')
