 ...
 * in the Recent Sessions bucket, open by default. Ensure it is open, then use its transient
...
async function expandSeededSessions(page: Page): Promise<void> {
  const bucket = page.getByText('Recent Sessions', { exact: true }).locator('..').locator('..')
  ...
  for (;;) {
    ...
    if (Date.now() > deadline) {
      throw new Error(`Recent Sessions bucket never revealed more than ${SEED_COUNT / 2} rows`)
    }
